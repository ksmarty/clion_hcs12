...
*               allow the data to ëstabilizeí and for the HD44780 to complete itís operation. The delay
...
void DispDataWr (INT8U data)
{
    ...
    PORTK |= (((data >> 4) & 0x0F) << LCD_LOWEST_DATA_BIT);             /* Write the UPPER nibble of ëdataí to D7..D4               */
    ...
    DispDly_uS(100);                                                    /* Delay for about 100 uS                                   */
    PORTK &=    LCD_BIT_RS;                                             /* Clear all PORTK data bits, RW = 0, preserve RS           */
    PORTK |=  ((data & 0x0F)       << LCD_LOWEST_DATA_BIT);             /* Write the LOWER nibble of ëdataí to D7..D4               */
    PORTK |=    LCD_BIT_E;                                              /* Set the E control line HIGH                              */
    DispDly_uS(100);                                                    /* Delay for about 100 uS                                   */
    PORTK &=   ~LCD_BIT_E;                                              /* Set the E control line LOW                               */
}
